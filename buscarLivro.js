...
export function buscarLivro(titulo) { //declara e exporta função para uso em outros arquivos
  ...
  for (let i = 0; i < dadosLivros.length; i++ ) { //percorre livros do array, i posição atual 

   const livro = dadosLivros[i]; //armazena livro posição atual na const
   
   if (livro.getTitulo() === titulo) { //compara livro atual com títuli informado
   ...
   console.log(`Status: ${livro.getDisponivel() ? 'Disponível' : 'Indisponível'}`) //status livro
  ...
  if (!livroEncontrado) { //variável valor false?
